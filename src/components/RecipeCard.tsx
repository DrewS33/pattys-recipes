...
// ============================================================
// RecipeCard: displays a single recipe in the grid
// ============================================================
...
// Warm, appetising gradients — softer than before
...
export default function RecipeCard({
  ...
  // Show up to 1 meaningful user tag (skip blank/whitespace entries)
  ...
  return (
    <div
      className={`
        ...
        ${isSelected
          ? 'shadow-card-lg ring-2 ring-primary-400/60 ring-offset-2 -translate-y-0.5'
    ...
    >
      ...
      <div className="relative h-52 overflow-hidden">
        {recipe.image ? (
          ...
          <div className={`w-full h-full bg-gradient-to-br ${gradient} flex items-center justify-center transition-transform duration-300 ease-out group-hover:scale-[1.04]`}>
            <span className="text-7xl drop-shadow-sm">{emoji}</span>
          </div>
        )}

        {/* Top-left badge: selected state OR quick-meal label */}
        {isSelected ? (
      ...
      <div className="px-5 pt-4 pb-5 flex flex-col flex-1 gap-3">
        ...
        {/* Name + rating */}
        <div>
          <h3 className="font-display text-[1.1rem] font-bold text-stone-800 leading-snug">
            {recipe.name}
          </h3>
        ...
        <div className="flex items-center gap-1.5 flex-wrap">
          ...
          {isFavorite && (
            <span className="text-xs font-medium px-2.5 py-1 rounded-full bg-yellow-50 text-yellow-600 border border-yellow-100">
              ❤️ Family Favorite
            </span>
          )}
          {firstTag && (
        ...
        {/* Info row — servings + total time (clean, no icon clutter) */}
        ...
        <div className="flex gap-2 pt-0.5">
          <button
            ...
            className="flex-1 py-2.5 px-3 bg-stone-50 hover:bg-stone-100 text-stone-500 hover:text-stone-700
                       rounded-xl text-sm font-medium transition-all duration-150 active:scale-[0.97]"
          >
            View Recipe
          </button>
          <button
